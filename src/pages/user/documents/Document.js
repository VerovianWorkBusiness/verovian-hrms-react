...
const Document = () => {
  const navigate = useNavigate()
    ...
    useEffect(() => {
      ...
      const getDocument = async () => {
        try {
          ...
          const response = await axios.get(`${process.env.REACT_APP_API_URL}/documents/${documentId}`, { headers })
          setDocumentPayload(response.data.data)
    ...
    }, [dispatch, navigate, documentsSelector.updatedDocument, documentId]);
    ...
    const triggerUpdateDocument = () => {
      ...
      dispatch(updateDocument(documentId, documentPayload))
    }
              ...
              <div className='w-full'>
                ...
                <div className='w-full'>
                  <div className='borderless px-6 long-text'>
                      <Wysiwyg 
                        fieldTitle=""
                        initialValue={documentPayload.documentBody || ''}
                ...
                <div className='my-8 flex flex-row-reverse items-center justify-between'>
                    ...
                    <FormButton buttonLabel={`Save document`} buttonAction={()=>{triggerUpdateDocument()}} processing={documentsSelector.updatingDocument} />
                    </div>
                </div>
              </div>
            </div>
