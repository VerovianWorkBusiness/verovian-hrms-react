...
import { createShift, fetchShiftDetails } from '../../../../store/actions/shiftActions'
...
const ShiftDetails = () => {
  ...
  useEffect(() => {
    if(shiftsState.updatedShift && shiftsState.updatedShift !== null) {
      dispatch({
        type: SET_SUCCESS_MESSAGE,
        payload: 'Shift updated successfully!'
      })
    }
    
    if(shiftPayload === null) {
      dispatch(fetchShiftDetails(shiftId))
    }

    if (shiftsState.shiftDetails !== null && shiftPayload === null) {
      // Only set shiftPayload when it's not already set to avoid infinite loops
      setShiftPayload(shiftsState.shiftDetails);
    }
    return () => {
        
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, navigate, shiftsState.createdShift, shiftId, shiftsState.updatedShift, shiftsState.shiftDetails]);


  const [shiftPayload, setShiftPayload] = useState(null);
  ...
  return (
        <UserLayout pageTitle={`Company`}>
            <CompanyPageLayout sectionTitle={'New Shift'}>
              {shiftsState.gettingShiftDetails && shiftPayload !== null ? 
                <Preloader preloadingText={`Loading shift details`} />
                :
                <div className='w-8/12 mx-auto'>
                    ...
                    <div className='my-8 w-full'>
                        <TextField
                            ...
                            fieldId="dept-name" 
                            inputType="text" 
                            preloadValue={shiftPayload?.name || ''}
                    ...
                    <div className='my-8 w-full'>
                        <TextareaField
                            ...
                            inputType="text" 
                            preloadValue={shiftPayload?.description || ''}
                            hasError={false} 
